  ...
  return (
    <>
      ...
      <AspectRatio
        ratio={aspectRatio}
        className={cn(
          "flex w-full items-center justify-center overflow-hidden rounded-lg border-2 border-border",
        )}
      >
        <div
        ...
        >
          {imageSource ? (
            <img
              className={cn(
                "h-full w-full border-2 border-border object-contain",
              )}
              src={imageSource as string}
              alt={t("imageUploadInput.uploadedImage")}
            />
          ) : (
          ...
          <Button
            ...
            className={cn(
              "absolute left-0 top-0 h-full w-full  min-w-[20px] border-2 border-border bg-muted opacity-0 transition-opacity duration-200 ease-in-out hover:opacity-90 focus:bg-muted",
            )}
            variant="outline"
            aria-label={t("imageUploadInput.uploadImage")}
          >
            <Icons.download />
          </Button>
        </div>
