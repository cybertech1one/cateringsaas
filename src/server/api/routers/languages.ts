...
export const languagesRouter = createTRPCRouter({
  ...
  changeMenuLanguages: privateProcedure
    .input(z.object({ languages: z.array(z.string().uuid()).max(50), menuId: z.string().uuid() }))
  ...
  changeDefaultLanguage: privateProcedure
    .input(z.object({ languageId: z.string().uuid(), menuId: z.string().uuid() }))
    .mutation(async ({ ctx, input }) => {
      const languages = await ctx.db.menuLanguages.findMany({
        where: {
          menus: {
      ...
      // Use sequential operations to avoid race condition between
      // setting all to false and setting one to true
      await ctx.db.menuLanguages.updateMany({
        where: {
          menus: {
            id: input.menuId,
            userId: ctx.user.id,
      ...
      await ctx.db.menuLanguages.update({
        where: {
          menus: {
            userId: ctx.user.id,
          },
