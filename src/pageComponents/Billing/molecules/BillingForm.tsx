    ...
    <>
      <AlertDialog>
        <AlertDialogTrigger asChild>
          <Button disabled={isSubscriptionLoading || isCancelSubscriptionLoading}>
            {t("billing.cancel")}
          </Button>
        </AlertDialogTrigger>
        <AlertDialogContent>
      ...
      <Card>
        ...
        <CardFooter className="flex flex-col items-start space-y-2 md:flex-row md:justify-between md:space-x-0">
          <button className={cn(buttonVariants())} disabled={true}>
            ...
            {isSubscribed ? t("billing.manageSubscription") : t("billing.upgradeToPro")}
          </button>
        </CardFooter>
      </Card>
    );
          ...
          <div className="flex gap-4">
            {subscriptionData?.status !== "cancelled" && (
              ...
              >
                ...
                {isSubscribed ? t("billing.updatePaymentDetails") : t("billing.upgradeToPro")}
              </button>
            )}
            <CancelButton />
          </div>
          {isSubscribed ? (
            <p className="rounded-full text-xs font-medium">
              {subscriptionData?.endsAt
                ? t("billing.planCancelledOn")
                : t("billing.planRenewsOn")}
